    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative">
      ...
      >
        ...
        <div className="glass-strong rounded-3xl p-8" style={{ borderColor: 'rgba(192,132,252,0.15)', boxShadow: '0 0 80px rgba(192,132,252,0.08)' }}>
          <h2 className="text-2xl font-grotesk font-bold mb-2 text-center text-white">Create account</h2>
          <p className="text-sm text-[var(--color-text-muted)] text-center mb-8">Start managing your tasks</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              ...
              <input
                ...
                autoComplete="name"
              />
              {errors.name && <span className="text-xs text-red-400 mt-1.5 block text-left">{errors.name.message}</span>}
            </div>
          ...
          <p className="text-sm text-[var(--color-text-muted)] text-center mt-6">
            ...
            <Link to="/login" className="text-[var(--color-accent)] hover:text-[var(--color-accent-light)] font-medium transition-colors">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
